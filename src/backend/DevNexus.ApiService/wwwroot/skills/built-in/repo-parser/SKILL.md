...
2. 获取到 Token 后，**立即写入** `.credential_cache.json` 进行缓存持久化（追加或更新对应域名记录）。
3. 然后使用 Token 继续执行解析。
...
- **凭证缓存**：只持久化到 `.credential_cache.json`，不在对话记录中留存原始 Token。
