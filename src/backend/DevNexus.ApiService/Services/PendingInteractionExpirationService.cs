...
public sealed class PendingInteractionExpirationService : BackgroundService
{
    ...
    private async Task ExpirePendingInteractionsAsync(CancellationToken cancellationToken)
    {
        ...
        var coordinator = scope.ServiceProvider.GetRequiredService<PendingInteractionExpirationCoordinator>();

        await coordinator.ExpireAsync(DateTime.UtcNow, cancellationToken);
    }
}
