...
/// </summary>
[ApiController]
[Route("api/v1/providers/note")]
[Authorize]
public class NoteProviderController : ControllerBase
{
    private readonly INoteProviderManagementService _service;
    private readonly ILogger<NoteProviderController> _logger;
    
    public NoteProviderController(
        INoteProviderManagementService service,
        ILogger<NoteProviderController> logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    /// </summary>
    [HttpGet]
    [ProducesResponseType(typeof(IEnumerable<NoteProviderResponse>), 200)]
    ...
    /// </summary>
    [HttpGet("{id:guid}")]
    [ProducesResponseType(typeof(NoteProviderResponse), 200)]
    ...
    [ProducesResponseType(typeof(NoteProviderResponse), 201)]
    [ProducesResponseType(400)]
    public async Task<IActionResult> CreateProvider(
        [FromBody] CreateNoteProviderRequest request,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public async Task<IActionResult> UpdateProvider(
        Guid id,
        [FromBody] UpdateNoteProviderRequest request,
        CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    [Authorize(Policy = AuthorizationPolicies.AdminOnly)]
    [ProducesResponseType(typeof(ValidateNoteProviderResponse), 200)]
    [ProducesResponseType(404)]
